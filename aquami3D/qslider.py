...
class QLabelRangeSlider(QtWidgets.QWidget):

    def __init__(self, label, rs_max=100, rs_range=None, parent=None):
        super(QLabelRangeSlider, self).__init__(parent)
        ...
        self.rs.setMin(0)
        self.rs.setMax(rs_max)
        self.rs.setRange(0, rs_max) if rs_range is None else self.rs.setRange(rs_range)
        self.rs.setBackgroundStyle('background: qlineargradient(x1:0, y1:0, x2:0, '
                              'y2:1, stop:0 #222, stop:1 #333);')
        self.rs.handle.setStyleSheet('background: qlineargradient(x1:0, y1:0, x2:0, '
                                'y2:1, stop:0 #282, stop:1 #393);')

        hbox = QtWidgets.QHBoxLayout()
        hbox.setSpacing(10)
        hbox.setStretch(1,10)
        hbox.addWidget(qlabel)
        hbox.addWidget(self.rs)
        hbox.addWidget(resetButton)

        self.setLayout(hbox)

    def reset(self):
        self.rs.setRange(0, self.rs.max())


class QSliceRange(QtWidgets.QWidget):
    valueChanged = QtCore.pyqtSignal(list)

    def __init__(self, parent=None, xmax=100, ymax=100, zmax=100):
        super(QSliceRange, self).__init__(parent)
        self.x_range = QLabelRangeSlider('X Slice:', xmax, None, self)
        self.y_range = QLabelRangeSlider('Y Slice:', ymax, None, self)
        self.z_range = QLabelRangeSlider('Z Slice:', zmax, None, self)
        self.indexes = []
        self.update_vals()

        self.x_range.rs.startValueChanged.connect(self.update_vals)
        self.x_range.rs.endValueChanged.connect(self.update_vals)
        self.y_range.rs.startValueChanged.connect(self.update_vals)
        self.y_range.rs.endValueChanged.connect(self.update_vals)
        self.z_range.rs.startValueChanged.connect(self.update_vals)
        self.z_range.rs.endValueChanged.connect(self.update_vals)

        vbox = QtWidgets.QVBoxLayout()
        vbox.setSpacing(1)
        vbox.addWidget(self.x_range)
        vbox.addWidget(self.y_range)
        vbox.addWidget(self.z_range)

        self.setLayout(vbox)

    def update_vals(self):
        xSliderRange = self.x_range.rs.getRange()
        ySliderRange = self.y_range.rs.getRange()
        zSliderRange = self.z_range.rs.getRange()
        self.indexes = [xSliderRange[0], xSliderRange[1],
                        ySliderRange[0], ySliderRange[1],
                        zSliderRange[0], zSliderRange[1]]
        self.valueChanged.emit(self.indexes)

    def set_range_maximums(self, maximums):
        # maximums = [xmax, ymax, zmax]
        self.x_range.rs.setMax(maximums[0])
        self.y_range.rs.setMax(maximums[1])
        self.z_range.rs.setMax(maximums[2])
        self.x_range.reset()
        self.y_range.reset()
        self.z_range.reset()
...
if __name__ == '__main__':
    ...
    rs = QSliceRange()
    rs.setFixedWidth(400)
    #rs.setFixedHeight(50)
    rs.show()
    app.exec_()
